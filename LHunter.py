...
class DatabaseManager:
    ...
    def verificar_lead_ja_abordado(self, telefone: str) -> tuple[str] | None:
        with self._get_connection() as conn:
            cursor = conn.execute("SELECT data_envio FROM leads_abordados WHERE telefone = ?", (telefone,))
            row = cursor.fetchone()
            return tuple(row) if row else None

    def verificar_ad_ja_coletado(self, ad_id: str) -> tuple[str] | None:
        with self._get_connection() as conn:
            cursor = conn.execute("SELECT data_coleta FROM ads_coletados WHERE ad_id = ?", (ad_id,))
            row = cursor.fetchone()
            return tuple(row) if row else None
    ...
    def salvar_lead_abordado(self, nome: str, telefone: str) -> bool:
        with self._write_lock:
            try:
                with self._get_connection() as conn:
                    conn.execute(
                        "INSERT OR REPLACE INTO leads_abordados (telefone, nome, data_envio) VALUES (?, ?, ?)",
    ...
    def salvar_ad_coletado(self, ad_id: str, titulo: str, link: str) -> bool:
        with self._write_lock:
            try:
                with self._get_connection() as conn:
                    conn.execute(
                        "INSERT OR REPLACE INTO ads_coletados (ad_id, titulo, link, data_coleta) VALUES (?, ?, ?, ?)",
    ...
    def salvar_grupo_whatsapp(self, link_grupo: str, nicho: str) -> bool:
        with self._write_lock:
            try:
                with self._get_connection() as conn:
                    conn.execute(
                        "INSERT OR REPLACE INTO grupos_whatsapp (link_grupo, nicho, data_coleta) VALUES (?, ?, ?)",
...
def criar_driver(com_imagens: bool = True, usar_perfil_sistema: bool = True) -> WebDriver:
    ...
    if usar_perfil_sistema:
        try:
            drv = BrowserFactory.create(headless=False, custom_profile=True, use_system_profile=True, options_args=custom_args)
            if drv:
                return drv
        except Exception as err:
            logger.warning(f"Perfil do sistema falhou: {err}")

    try:
        drv = BrowserFactory.create(headless=False, custom_profile=True, use_system_profile=False, options_args=custom_args)
        if drv:
            return drv
    except Exception:
        pass
    ...
    try:
        drv = BrowserFactory.create(headless=False, custom_profile=False, use_system_profile=False, options_args=custom_args)
        if drv:
            return drv
    except Exception:
        pass

    return BrowserFactory.create(preferred_browser=SupportedBrowsers.CHROME, headless=False, custom_profile=False, use_system_profile=False, options_args=custom_args)
...
def abrir_url_em_nova_aba(driver: WebDriver | None, url: str) -> None:
    ...
    try:
        ...
        if "data:," in current_url or "about:blank" in current_url or "msn.com" in current_url or not current_url:
            driver.get(url)
        else:
...
def _dismiss_google_consent(driver: WebDriver | None) -> None:
    ...
    try:
        driver.execute_script("""
            var msBtns = document.querySelectorAll('button[id*="accept"], button[class*="accept"], #accept-all, .bep-accept');
            for (var i = 0; i < msBtns.length; i++) { try { msBtns[i].click(); } catch(e) {} }
        """)
        btns = driver.find_elements(By.XPATH, '//button[contains(., "Aceitar") or contains(., "Concordo") or contains(., "Accept all")]')
        if btns:
...
class GeminiClientManager:
    ...
    def otimizar_termo_pesquisa(self, termo_base: str) -> str:
        if not self.ativo:
            return termo_base
        prompt = f"Atue como um Agente Otimizador de SEO B2B. Melhore o termo '{termo_base}' para encontrar empresas de alto valor. Responda APENAS com a nova frase."
    ...
    def otimizar_copywriting_whatsapp(self, nome_empresa: str, nicho: str, proposta: str) -> str:
        if not self.ativo:
            return f"Olá! Vi o trabalho da '{nome_empresa}' no segmento de {nicho}. {proposta} Teria 2 minutos hoje?"
        prompt = f"Escreva uma mensagem persuasiva B2B de no máximo 4 linhas para '{nome_empresa}' (nicho: {nicho}). Proposta: '{proposta}'. Sem formalismos e com CTA direta."
        res = self.gerar_conteudo(prompt)
    ...
    def gerar_saudacao_aquecimento(self, nome_grupo: str) -> str:
        ...
        prompt = f"Gere uma curta mensagem de saudação (1 linha) para enviar ao entrar em um grupo de WhatsApp sobre '{nome_grupo}'."
        res = self.gerar_conteudo(prompt)
...
class LeadHunterProApp(ctk.CTk):
    MAPS_CIDADES_VIZINHAS = {
        ...
        "goiânia": ["Aparecida de Goiânia GO", "Senador Canedo GO", "Trindade GO", "Anápolis GO"],
        "palmas": ["Taquaralto TO", "Porto Nacional TO", "Paraíso do Tocantins TO"],
        "são paulo": ["Guarulhos SP", "Campinas SP", "São Bernardo do Campo SP", "Osasco SP"],
        "rio de janeiro": ["Niterói RJ", "Duque de Caxias RJ", "Nova Iguaçu RJ"],
        "curitiba": ["São José dos Pinhais PR", "Colombo PR", "Araucária PR"],
        "belo horizonte": ["Contagem MG", "Betim MG", "Nova Lima MG"],
        "brasília": ["Taguatinga DF", "Ceilândia DF", "Águas Claras DF", "Luziânia GO"],
    }
    ...
    def gerar_pool_buscas_regional(self, termo_original: str) -> list[str]:
        ...
        nicho = match_nicho[0].strip() if len(match_nicho) > 1 and match_nicho[0].strip() else termo_original.strip()
    ...
    def __init__(self) -> None:
        ...
        self.title("⚡ LEAD HUNTER PRO v3.0 — Gemini AI & Lit Hunter Auto Pipeline")
        ...
        self.log(f"🌐 Navegador Padrão do Usuário: {nome_browser_padrao}")
    ...
    def validar_e_filtrar_celular(self, tel_limpo: str) -> bool:
        if SUPORTA_RUST and _rust_engine is not None and hasattr(_rust_engine, "validar_celular_brasileiro"):
            try:
                ...
                pass
        nacional = tel_limpo[2:] if tel_limpo.startswith("55") and len(tel_limpo) >= 12 else tel_limpo
    ...
    def definir_modo_projeto(self, ativar_lit: bool) -> None:
        ...
        if self.modo_lit_hunter:
            ...
            self.btn_iniciar_coleta.configure(text="🔥 1. MINERAR E ENTRAR AUTO", fg_color="#D97706", hover_color="#B45309")
    ...
    def criar_layout_dashboard(self) -> None:
        ...
        self.col1_frame = ctk.CTkFrame(self.center_dashboard, fg_color="#0F172A", corner_radius=14, width=460, border_width=1, border_color="#1E293B")
        self.col1_frame.pack(side="left", fill="both", expand=False, padx=(0, 12), pady=0)
        self.col1_frame.pack_propagate(False)

        self.col2_frame = ctk.CTkFrame(self.center_dashboard, fg_color="#0F172A", corner_radius=14, border_width=1, border_color="#1E293B")
        self.col2_frame.pack(side="left", fill="both", expand=True, padx=(0, 12), pady=0)

        self.col3_frame = ctk.CTkFrame(self.center_dashboard, fg_color="#0F172A", corner_radius=14, width=400, border_width=1, border_color="#1E293B")
    ...
    def _criar_header_e_kpis(self) -> None:
        self.top_header = ctk.CTkFrame(self.main_panel, fg_color="#0F172A", corner_radius=14, height=62, border_width=1, border_color="#1E293B")
        ...
        ctk.CTkLabel(brand_box, text="⚡ LEAD HUNTER PRO", font=("Arial Bold", 18), text_color="#38BDF8").pack(side="left")
        ctk.CTkLabel(brand_box, text=" | ENTERPRISE SUITE v3.0", font=("Arial Bold", 11), text_color="#64748B").pack(side="left", padx=(6, 0))

        mode_box = ctk.CTkFrame(self.top_header, fg_color="transparent")
        mode_box.pack(side="left", padx=(24, 0))

        self.btn_mode_lead = ctk.CTkButton(mode_box, text="⚡ LEAD HUNTER PRO", font=("Arial Bold", 11), fg_color="#0284C7", hover_color="#0369A1", width=160, height=32, corner_radius=8, command=lambda: self.definir_modo_projeto(False))
        self.btn_mode_lead.pack(side="left", padx=(0, 8))

        self.btn_mode_lit = ctk.CTkButton(mode_box, text="🔥 LIT HUNTER (AQUECEDOR)", font=("Arial Bold", 11), fg_color="#1E293B", hover_color="#334155", text_color="#94A3B8", width=190, height=32, corner_radius=8, command=lambda: self.definir_modo_projeto(True))
        self.btn_mode_lit.pack(side="left")

        self.badge_status = ctk.CTkFrame(self.top_header, fg_color="#060810", corner_radius=20, height=36, width=210, border_width=1, border_color="#334155")
        self.badge_status.pack(side="right", padx=18)
        self.badge_status.pack_propagate(False)

        self.lbl_status_sistema = ctk.CTkLabel(self.badge_status, text="● SISTEMA EM ESPERA", font=("Arial Bold", 11), text_color="#EF4444")
        ...
        c1 = ctk.CTkFrame(self.kpi_bar, fg_color="#0F172A", corner_radius=12, border_width=1, border_color="#1E293B")
        c1.pack(side="left", fill="x", expand=True, padx=(0, 8))
        ctk.CTkLabel(c1, text="LEADS / GRUPOS CAPTADOS", font=("Arial Bold", 10), text_color="#94A3B8").pack(anchor="w", padx=12, pady=(6, 0))
        self.lbl_stat_coletados = ctk.CTkLabel(c1, text="0", font=("Arial Bold", 20), text_color="#38BDF8")
        self.lbl_stat_coletados.pack(anchor="w", padx=12, pady=(0, 2))
        self.progress_coletados = ctk.CTkProgressBar(c1, height=5, corner_radius=3, fg_color="#060810", progress_color="#38BDF8")
        self.progress_coletados.set(0.0)
        self.progress_coletados.pack(fill="x", padx=12, pady=(0, 6))

        c2 = ctk.CTkFrame(self.kpi_bar, fg_color="#0F172A", corner_radius=12, border_width=1, border_color="#1E293B")
        c2.pack(side="left", fill="x", expand=True, padx=(4, 8))
        ctk.CTkLabel(c2, text="DISPAROS / AQUECIMENTOS", font=("Arial Bold", 10), text_color="#94A3B8").pack(anchor="w", padx=12, pady=(6, 0))
        self.lbl_stat_enviados = ctk.CTkLabel(c2, text="0", font=("Arial Bold", 20), text_color="#10B981")
        self.lbl_stat_enviados.pack(anchor="w", padx=12, pady=(0, 2))
        self.progress_enviados = ctk.CTkProgressBar(c2, height=5, corner_radius=3, fg_color="#060810", progress_color="#10B981")
        self.progress_enviados.set(0.0)
        self.progress_enviados.pack(fill="x", padx=12, pady=(0, 6))

        c3 = ctk.CTkFrame(self.kpi_bar, fg_color="#0F172A", corner_radius=12, border_width=1, border_color="#1E293B")
        c3.pack(side="left", fill="x", expand=True, padx=(4, 8))
        ctk.CTkLabel(c3, text="COTAÇÃO DÓLAR (USD/BRL)", font=("Arial Bold", 10), text_color="#94A3B8").pack(anchor="w", padx=12, pady=(8, 0))
        self.lbl_kpi_dolar = ctk.CTkLabel(c3, text="R$ 5.08", font=("Arial Bold", 20), text_color="#F59E0B")
        self.lbl_kpi_dolar.pack(anchor="w", padx=12, pady=(0, 8))

        c4 = ctk.CTkFrame(self.kpi_bar, fg_color="#0F172A", corner_radius=12, border_width=1, border_color="#1E293B")
        c4.pack(side="left", fill="x", expand=True, padx=(4, 0))
        ctk.CTkLabel(c4, text="ENGINE DE IA GEMINI", font=("Arial Bold", 10), text_color="#94A3B8").pack(anchor="w", padx=12, pady=(8, 0))
        self.lbl_kpi_engine = ctk.CTkLabel(c4, text="GEMINI-2.0-FLASH", font=("Arial Bold", 16), text_color="#A855F7")
    ...
    def _montar_coluna_1_controles(self) -> None:
        ...
        ctk.CTkLabel(api_frame, text="🔑 Google Gemini AI Key", font=("Arial Bold", 11), text_color="#38BDF8").pack(anchor="w", padx=10, pady=(6, 2))

        row_api = ctk.CTkFrame(api_frame, fg_color="transparent")
        row_api.pack(fill="x", padx=10, pady=(0, 4))

        self.entry_api_key = ctk.CTkEntry(row_api, height=30, font=("Arial", 10), fg_color="#060810", border_color="#334155", placeholder_text="Cole sua API Key aqui...")
        self.entry_api_key.pack(side="left", fill="x", expand=True, padx=(0, 6))

        ctk.CTkButton(row_api, text="Validar", width=70, height=30, font=("Arial Bold", 10), fg_color="#0284C7", hover_color="#0369A1", command=self.testar_conexao_api).pack(side="right")

        self.lbl_api_status = ctk.CTkLabel(api_frame, text="🔴 Status da IA: Aguardando verificação", font=("Arial Italic", 10), text_color="#EF4444")
        self.lbl_api_status.pack(anchor="w", padx=10, pady=(0, 6))

        self.tabview = ctk.CTkTabview(self.col1_frame, fg_color="#060810", segmented_button_fg_color="#0F172A", segmented_button_selected_color="#0284C7", segmented_button_unselected_color="#1E293B", height=270, corner_radius=10)
        ...
        # Aba 1: Maps
        ctk.CTkLabel(self.tab_comercial, text="TERMO DE PESQUISA (MAPS)", font=("Arial Bold", 10), text_color="#94A3B8").pack(anchor="w")
        self.entry_termo_comercial = ctk.CTkEntry(self.tab_comercial, height=28, font=("Arial", 11), fg_color="#0F172A", border_color="#334155")
        self.entry_termo_comercial.insert(0, "Lojas em Anápolis Goias")
        self.entry_termo_comercial.pack(fill="x", pady=(2, 4))

        ctk.CTkLabel(self.tab_comercial, text="OFERTA / MENSAGEM BASE", font=("Arial Bold", 10), text_color="#38BDF8").pack(anchor="w")
        self.txt_msg_comercial = ctk.CTkTextbox(self.tab_comercial, height=50, font=("Arial", 10), fg_color="#0F172A", border_color="#334155", border_width=1)
        self.txt_msg_comercial.pack(fill="x", pady=(2, 0))
        self.txt_msg_comercial.insert("0.0", "Oferecer criação de Landing Pages profissionais e estratégias de vendas e-commerce.")

        # Aba 2: Busca Web
        ctk.CTkLabel(self.tab_diretorios, text="NICHO OU PROFISSÃO", font=("Arial Bold", 10), text_color="#94A3B8").pack(anchor="w")
        self.entry_termo_avancado = ctk.CTkEntry(self.tab_diretorios, height=26, font=("Arial", 11), fg_color="#0F172A", border_color="#334155")
        self.entry_termo_avancado.insert(0, "Fotógrafo")
        self.entry_termo_avancado.pack(fill="x", pady=(1, 3))

        ctk.CTkLabel(self.tab_diretorios, text="CIDADE E ESTADO", font=("Arial Bold", 10), text_color="#38BDF8").pack(anchor="w")
        self.entry_cidade_avancado = ctk.CTkEntry(self.tab_diretorios, height=26, font=("Arial", 11), fg_color="#0F172A", border_color="#334155")
        self.entry_cidade_avancado.insert(0, "Anápolis GO")
        self.entry_cidade_avancado.pack(fill="x", pady=(1, 3))

        self.txt_msg_diretorios = ctk.CTkTextbox(self.tab_diretorios, height=35, font=("Arial", 10), fg_color="#0F172A", border_color="#334155", border_width=1)
        self.txt_msg_diretorios.pack(fill="x", pady=0)
        self.txt_msg_diretorios.insert("0.0", "Oferecer soluções em inteligência artificial para otimizar os serviços.")

        # Aba 3: Meta Ads
        ctk.CTkLabel(self.tab_meta_ads, text="ESPIONAGEM META ADS (EX: 'emagrecimento')", font=("Arial Bold", 10), text_color="#94A3B8").pack(anchor="w")
        self.entry_termo_ads = ctk.CTkEntry(self.tab_meta_ads, height=28, font=("Arial", 11), fg_color="#0F172A", border_color="#334155")
        ...
        self.entry_qtd_ads = ctk.CTkEntry(f_qtd, height=28, font=("Arial", 11), fg_color="#0F172A", border_color="#334155")
        ...
        self.entry_tempo_mineracao = ctk.CTkEntry(f_tempo, height=28, font=("Arial", 11), fg_color="#0F172A", border_color="#334155")
        self.entry_tempo_mineracao.insert(0, "10")
        self.entry_tempo_mineracao.pack(fill="x")

        ctk.CTkButton(self.tab_meta_ads, text="🎯 PESQUISAR TERMO ÚNICO", font=("Arial Bold", 10), fg_color="#A855F7", hover_color="#7E22CE", height=28, command=self.disparar_thread_ads_library).pack(fill="x", pady=(4, 2))
        ctk.CTkButton(self.tab_meta_ads, text="🚀 AUTO-MINERAR MULTI-NICHOS POR TEMPO", font=("Arial Bold", 10), fg_color="#D97706", hover_color="#B45309", height=30, command=self.disparar_thread_auto_mineracao).pack(fill="x", pady=2)

        # Aba 4: Comparador
        ctk.CTkLabel(self.tab_comparador, text="PRODUTO DIGITAL (EX: 'CapCut', 'Canva')", font=("Arial Bold", 10), text_color="#94A3B8").pack(anchor="w")
        self.entry_termo_comparar = ctk.CTkEntry(self.tab_comparador, height=28, font=("Arial", 11), fg_color="#0F172A", border_color="#334155")
        self.entry_termo_comparar.insert(0, "CapCut")
        self.entry_termo_comparar.pack(fill="x", pady=(2, 6))

        ctk.CTkButton(self.tab_comparador, text="🔎 COMPARAR PREÇOS (PLATI vs Z2U vs GGMAX)", font=("Arial Bold", 11), fg_color="#10B981", hover_color="#059669", height=32, command=self.disparar_thread_comparador).pack(fill="x", pady=2)

        # Aba 5: Afiliados
        row_afiliados_plat = ctk.CTkFrame(self.tab_afiliados, fg_color="transparent")
        row_afiliados_plat.pack(fill="x", pady=(0, 2))
        ctk.CTkLabel(row_afiliados_plat, text="PLATAFORMA DE AFILIADOS", font=("Arial Bold", 10), text_color="#38BDF8").pack(anchor="w")
        self.opt_plataforma_afiliados = ctk.CTkOptionMenu(row_afiliados_plat, values=["Todas as Plataformas", "Shopee", "Amazon", "Mercado Livre"], fg_color="#0F172A", button_color="#0284C7", button_hover_color="#0369A1", dropdown_fg_color="#0F172A", font=("Arial Bold", 10), height=26)
        self.opt_plataforma_afiliados.pack(fill="x", pady=(1, 3))

        ctk.CTkLabel(self.tab_afiliados, text="NICHO / PRODUTO (EX: 'fones bluetooth')", font=("Arial Bold", 10), text_color="#94A3B8").pack(anchor="w")
        self.entry_termo_afiliados = ctk.CTkEntry(self.tab_afiliados, height=26, font=("Arial", 11), fg_color="#0F172A", border_color="#334155")
        self.entry_termo_afiliados.insert(0, "fones bluetooth")
        self.entry_termo_afiliados.pack(fill="x", pady=(1, 3))

        # Aba 6: Lit Hunter
        ctk.CTkLabel(self.tab_lit_hunter, text="NICHO / CIDADE DOS GRUPOS (EX: 'vendas', 'brasilia')", font=("Arial Bold", 10), text_color="#F59E0B").pack(anchor="w")
        self.entry_termo_lit = ctk.CTkEntry(self.tab_lit_hunter, height=28, font=("Arial", 11), fg_color="#0F172A", border_color="#334155")
        self.entry_termo_lit.insert(0, "vendas e-commerce produtos")
        self.entry_termo_lit.pack(fill="x", pady=(2, 4))

        ctk.CTkLabel(self.tab_lit_hunter, text="QTD DE GRUPOS ALVO", font=("Arial Bold", 10), text_color="#38BDF8").pack(anchor="w")
        self.entry_qtd_lit = ctk.CTkEntry(self.tab_lit_hunter, height=28, font=("Arial", 11), fg_color="#0F172A", border_color="#334155")
        self.entry_qtd_lit.insert(0, "15")
        self.entry_qtd_lit.pack(fill="x", pady=(2, 6))

        ctk.CTkButton(self.tab_lit_hunter, text="🔥 1. MINERAR E ENTRAR AUTO", font=("Arial Bold", 10), fg_color="#D97706", hover_color="#B45309", height=30, command=self.disparar_thread_minerar_lit).pack(fill="x", pady=2)
        ctk.CTkButton(self.tab_lit_hunter, text="🚀 2. INICIAR AQUECIMENTO (ENTRAR NOS GRUPOS)", font=("Arial Bold", 10), fg_color="#10B981", hover_color="#059669", height=32, command=self.disparar_thread_aquecer_lit).pack(fill="x", pady=2)
        ...
        ctk.CTkLabel(sub_m, text="META LEADS", font=("Arial Bold", 10), text_color="#94A3B8").pack(anchor="w")
        self.entry_meta = ctk.CTkEntry(sub_m, height=28, font=("Arial", 11), fg_color="#060810", border_color="#334155")
        ...
        ctk.CTkLabel(sub_h, text="HORÁRIO (HH:MM)", font=("Arial Bold", 10), text_color="#38BDF8").pack(anchor="w")
        self.entry_horario = ctk.CTkEntry(sub_h, height=28, font=("Arial", 11), fg_color="#060810", border_color="#334155")
        ...
        self.btn_add_img = ctk.CTkButton(btn_action_frame, text="📁 Anexar Imagens Exemplo", font=("Arial", 11), fg_color="#1E293B", hover_color="#334155", text_color="#F8FAFC", height=30, corner_radius=8, command=self.selecionar_imagens)
        self.btn_add_img.pack(fill="x", pady=2)

        self.lbl_qtd_imagens = ctk.CTkLabel(btn_action_frame, text="Nenhuma imagem anexada", font=("Arial Italic", 9), text_color="#64748B")
        self.lbl_qtd_imagens.pack(anchor="w", pady=(0, 4))

        self.btn_iniciar_coleta = ctk.CTkButton(btn_action_frame, text="🔍 1. INICIAR COLETA DE LEADS", font=("Arial Bold", 11), fg_color="#0284C7", hover_color="#0369A1", height=34, command=self.disparar_thread_coleta)
        self.btn_iniciar_coleta.pack(fill="x", pady=2)

        self.btn_iniciar_disparos = ctk.CTkButton(btn_action_frame, text="🚀 2. INICIAR DISPAROS COM IA", font=("Arial Bold", 11), fg_color="#10B981", hover_color="#059669", height=34, command=self.disparar_thread_envio)
        self.btn_iniciar_disparos.pack(fill="x", pady=2)

        self.btn_ativar_agendamento = ctk.CTkButton(btn_action_frame, text="⏰ ATIVAR AGENDAMENTO AUTO", font=("Arial Bold", 11), fg_color="#D97706", hover_color="#B45309", height=32, command=self.toggle_agendamento)
        self.btn_ativar_agendamento.pack(fill="x", pady=2)

        self.btn_parar = ctk.CTkButton(btn_action_frame, text="⏹ PARAR TUDO", font=("Arial Bold", 11), fg_color="#DC2626", hover_color="#B91C1C", height=30, command=self.parar_automacao)
    ...
    def _montar_coluna_2_resultados(self) -> None:
        ...
        ctk.CTkLabel(top_bar_col2, text="DATA STREAM & CARDS DE RESULTADOS", font=("Arial Bold", 12), text_color="#94A3B8").pack(side="left")

        btn_box = ctk.CTkFrame(top_bar_col2, fg_color="transparent")
        btn_box.pack(side="right")

        ctk.CTkButton(btn_box, text="📥 Exportar (.xlsm/Excel)", width=130, height=26, font=("Arial Bold", 10), fg_color="#10B981", hover_color="#059669", command=self.exportar_leads_excel).pack(side="left", padx=(0, 4))
        ctk.CTkButton(btn_box, text="Marcar Todos", width=90, height=26, font=("Arial Bold", 10), fg_color="#1E293B", command=self.marcar_todos_leads).pack(side="left", padx=(0, 4))
        ctk.CTkButton(btn_box, text="Desmarcar", width=80, height=26, font=("Arial Bold", 10), fg_color="#1E293B", command=self.desmarcar_todos_leads).pack(side="left", padx=(0, 4))
        ctk.CTkButton(btn_box, text="🧹 Limpar Tela", width=90, height=26, font=("Arial Bold", 10), fg_color="#DC2626", hover_color="#B91C1C", command=self.limpar_resultados).pack(side="left")

        self.scroll_leads = ctk.CTkScrollableFrame(self.col2_frame, fg_color="#060810", border_color="#1E293B", border_width=1, corner_radius=10)
    ...
    def _montar_coluna_3_telemetria(self) -> None:
        ...
        ctk.CTkLabel(zap_card, text="🟢 WHATSAPP DESKTOP PRONTO", font=("Arial Bold", 11), text_color="#10B981").pack(anchor="w", padx=12, pady=(10, 0))
        ctk.CTkLabel(zap_card, text="Integração de Cotação R$ & Trava Antiduplicidade Ativa.", font=("Arial", 10), text_color="#94A3B8").pack(anchor="w", padx=12, pady=(0, 8))

        self.card_timer_pausa = ctk.CTkFrame(self.col3_frame, fg_color="#1E293B", corner_radius=10, border_width=1, border_color="#334155")
        ...
        ctk.CTkLabel(top_timer_row, text="🛡️ PAUSA DE SEGURANÇA (ANTI-BAN)", font=("Arial Bold", 10), text_color="#F59E0B").pack(side="left")
        self.lbl_timer_pausa = ctk.CTkLabel(top_timer_row, text="EM ESPERA", font=("Arial Bold", 11), text_color="#64748B")
        self.lbl_timer_pausa.pack(side="right")

        self.progress_timer_pausa = ctk.CTkProgressBar(self.card_timer_pausa, height=5, corner_radius=3, fg_color="#060810", progress_color="#F59E0B")
        self.progress_timer_pausa.set(0.0)
        self.progress_timer_pausa.pack(fill="x", padx=12, pady=(0, 8))

        ctk.CTkLabel(self.col3_frame, text="CONSOLE DE EXECUÇÃO EM TEMPO REAL", font=("Arial Bold", 11), text_color="#94A3B8").pack(anchor="w", padx=14, pady=(4, 4))

        self.txt_console = ctk.CTkTextbox(self.col3_frame, font=("Consolas", 10), fg_color="#060810", text_color="#38BDF8", border_color="#1E293B", border_width=1, corner_radius=10)
        self.txt_console.pack(fill="both", expand=True, padx=14, pady=(0, 14))
        self.txt_console.insert("0.0", f">>> [LEAD HUNTER PRO v3.0 Enterprise — Gemini Auto-Pipeline Engine]\n>>> Base: {self._base_path}\n")
        self.txt_console.configure(state="disabled")

    def _criar_rodape_enterprise(self) -> None:
        self.footer_bar = ctk.CTkFrame(self, fg_color="#0F172A", corner_radius=0, height=36, border_width=1, border_color="#1E293B")
        self.footer_bar.pack(side="bottom", fill="x", padx=0, pady=0)
        self.footer_bar.pack_propagate(False)

        self.lbl_footer_tempo = ctk.CTkLabel(self.footer_bar, text="AUTOMAÇÃO EM ESPERA", font=("Arial Bold", 10), text_color="#10B981")
        self.lbl_footer_tempo.pack(side="left", padx=20)

        ctk.CTkLabel(self.footer_bar, text="Multi-Marketplace Intelligence & Automated Outreach Engine", font=("Arial", 10), text_color="#64748B").pack(side="left", expand=True)

        self.lbl_footer_data = ctk.CTkLabel(self.footer_bar, text="", font=("Arial Bold", 10), text_color="#CBD5E1")
        self.lbl_footer_data.pack(side="right", padx=20)

    # ── MÉTODOS DE DISPARO EM THREAD ──────────────────────────────────────────
    ...
    def disparar_thread_aquecer_lit(self) -> None:
        if not self._tentar_iniciar_automacao():
            return
        self.tempo_inicio_automacao = time.time()
        self._ui_safe(lambda: self.lbl_status_sistema.configure(text="● AQUECENDO WHATSAPP...", text_color="#10B981"))
    ...
    def disparar_thread_afiliados_auto(self) -> None:
        if not self._tentar_iniciar_automacao():
            return
        self.tempo_inicio_automacao = time.time()
        self._ui_safe(lambda: self.lbl_status_sistema.configure(text="● AUTO-MINERANDO AFILIADOS...", text_color="#D97706"))
    ...
    def disparar_thread_auto_mineracao(self) -> None:
        if not self._tentar_iniciar_automacao():
            return
        self.tempo_inicio_automacao = time.time()
        self._ui_safe(lambda: self.lbl_status_sistema.configure(text="● AUTO-MINERANDO POR TEMPO...", text_color="#F59E0B"))
    ...
    def disparar_thread_envio(self) -> None:
        ...
        if self.modo_lit_hunter or aba_atual == "🔥 Lit Hunter":
            ...
            if not self.leads_dados:
                ...
                return
            self.tempo_inicio_automacao = time.time()
            self._ui_safe(lambda: self.lbl_status_sistema.configure(text="● ENVIANDO COM IA...", text_color="#10B981"))
    ...
    def exportar_leads_excel(self) -> None:
        ...
        caminho_arquivo = filedialog.asksaveasfilename(
            ...
            filetypes=[("Planilha Excel (.xlsm)", "*.xlsm"), ("Planilha Excel (.xlsx)", "*.xlsx"), ("Arquivo CSV (Google Contacts)", "*.csv")]
        )
        ...
        try:
            if SUPORTA_OPENPYXL and (caminho_arquivo.endswith(".xlsx") or caminho_arquivo.endswith(".xlsm")):
                ...
                for item in self.leads_dados:
                    ws.append([item["nome"], item["telefone"], datetime.now().strftime("%d/%m/%Y"), item["nome"], f"+{item['telefone']}"])

                wb.save(caminho_arquivo)
                self.log(f"✔ Planilha exportada em: {caminho_arquivo}")
            else:
                with open(caminho_arquivo, "w", encoding="utf-8-sig", newline="") as f:
                    ...
                    for item in self.leads_dados:
                        writer.writerow([item["nome"], item["telefone"], datetime.now().strftime("%d/%m/%Y"), item["nome"], f"+{item['telefone']}"])
                self.log(f"✔ Tabela salva em: {caminho_arquivo}")
    ...
    def adicionar_lead_na_lista(self, nome_loja: str, telefone: str) -> bool:
        try:
            if not self.validar_e_filtrar_celular(telefone) or any(ld["telefone"] == telefone for ld in self.leads_dados):
                return False

            registro_antigo = self.db.verificar_lead_ja_abordado(telefone)
            var_status = ctk.BooleanVar(value=not bool(registro_antigo))

            frame_linha = ctk.CTkFrame(self.scroll_leads, fg_color="#0F172A", height=52, corner_radius=8, border_width=1, border_color="#1E293B")
            frame_linha.pack(fill="x", pady=4)

            texto_label = f"👤 {nome_loja}\n📞 {telefone}" + (f" (⚠️ Já abordado em {registro_antigo[0]})" if registro_antigo else "")
            cor_texto = "#FACC15" if registro_antigo else "#F8FAFC"

            ctk.CTkCheckBox(frame_linha, text=texto_label, variable=var_status, font=("Arial", 10), text_color=cor_texto, fg_color="#0284C7").pack(side="left", padx=12, pady=6, anchor="w")

            self.leads_dados.append({"nome": nome_loja, "telefone": telefone, "var": var_status, "frame": frame_linha})
            return True
        except Exception as err:
            self.log(f"✖ Erro ao adicionar lead: {err}")
            return False

    def adicionar_grupo_lit_na_lista(self, titulo_grupo: str, link_grupo: str) -> None:
        frame_linha = ctk.CTkFrame(self.scroll_leads, fg_color="#0F172A", height=75, corner_radius=10, border_width=1, border_color="#F59E0B")
        frame_linha.pack(fill="x", pady=4)
        ...
        ctk.CTkLabel(info_box, text=f"🔥 {titulo_grupo}", font=("Arial Bold", 11), text_color="#F59E0B", anchor="w").pack(anchor="w")
        ctk.CTkLabel(info_box, text=f"🔗 Link Público: {link_grupo[:60]}...", font=("Consolas", 9), text_color="#94A3B8", anchor="w").pack(anchor="w")

        ctk.CTkButton(frame_linha, text="Entrar no Grupo", width=110, height=28, font=("Arial Bold", 10), fg_color="#D97706", hover_color="#B45309", command=lambda u=link_grupo: self.abrir_link_anuncio_seguro(u)).pack(side="right", padx=12)

        self.grupos_coletados.append({"titulo": titulo_grupo, "link": link_grupo, "frame": frame_linha})

    def adicionar_anuncio_na_lista(self, titulo_anuncio: str, img_url: str, link_ads: str, taxa_conversao: float, ja_coletado_data: str | None = None) -> None:
        frame_linha = ctk.CTkFrame(self.scroll_leads, fg_color="#0F172A", height=115, corner_radius=10, border_width=1, border_color="#334155")
        frame_linha.pack(fill="x", pady=5)
        frame_linha.pack_propagate(False)

        lbl_placeholder = ctk.CTkLabel(frame_linha, text="🎯\nMeta Ad", width=90, height=90, fg_color="#1E293B", corner_radius=8, font=("Arial Bold", 11), text_color="#38BDF8")
        lbl_placeholder.pack(side="left", padx=8, pady=8)

        info_box = ctk.CTkFrame(frame_linha, fg_color="transparent")
        info_box.pack(side="left", fill="both", expand=True, padx=6, pady=6)
        ctk.CTkLabel(info_box, text=f"🎯 {titulo_anuncio}", font=("Arial Bold", 11), text_color="#38BDF8", anchor="w").pack(anchor="w")

        badge = f"⚠️ Coletado em {ja_coletado_data} | Conversão Est.: {taxa_conversao}%" if ja_coletado_data else f"🌟 NOVO / Validado | Conversão Est.: {taxa_conversao}%"
        ctk.CTkLabel(info_box, text=badge, font=("Arial Bold", 10), text_color="#FACC15" if ja_coletado_data else "#10B981", anchor="w").pack(anchor="w")

        row_link = ctk.CTkFrame(info_box, fg_color="transparent")
        row_link.pack(anchor="w", fill="x", pady=(2, 0))

        ctk.CTkLabel(row_link, text=f"🔗 Direct: {link_ads[:45]}...", font=("Consolas", 9), text_color="#94A3B8", anchor="w").pack(side="left", padx=(0, 6))
        ctk.CTkButton(row_link, text="Abrir no Meta", width=110, height=22, font=("Arial Bold", 9), fg_color="#A855F7", hover_color="#7E22CE", command=lambda u=link_ads: self.abrir_link_anuncio_seguro(u)).pack(side="left")
                    ...
                    try:
                        with io.BytesIO(img_bytes) as img_data:
                            with Image.open(img_data) as pil_img:
                                ...
                                ctk_img = ctk.CTkImage(light_image=pil_resized, dark_image=pil_resized, size=(90, 90)) if hasattr(ctk, "CTkImage") else ImageTk.PhotoImage(pil_resized)
                        self.after(0, lambda: lbl_placeholder.configure(image=ctk_img, text=""))
                    except Exception:
                        pass
            threading.Thread(target=_carregar_imagem_async, daemon=True).start()

    def adicionar_produto_afiliado_na_lista(self, titulo: str, plataforma: str, preco_brl: float, img_url: str, link_produto: str, taxa_conversao: float) -> None:
        frame_linha = ctk.CTkFrame(self.scroll_leads, fg_color="#0F172A", height=105, corner_radius=10, border_width=1, border_color="#EAB308")
        frame_linha.pack(fill="x", pady=5)
        frame_linha.pack_propagate(False)

        lbl_placeholder = ctk.CTkLabel(frame_linha, text="🛍️\nProduto", width=80, height=80, fg_color="#1E293B", corner_radius=8, font=("Arial Bold", 10), text_color="#EAB308")
        lbl_placeholder.pack(side="left", padx=8, pady=8)

        info_box = ctk.CTkFrame(frame_linha, fg_color="transparent")
        info_box.pack(side="left", fill="both", expand=True, padx=6, pady=6)
        ctk.CTkLabel(info_box, text=f"🛍️ {titulo}", font=("Arial Bold", 11), text_color="#F8FAFC", anchor="w").pack(anchor="w")

        badge = f"🏷️ {plataforma} | 🇧🇷 Preço: R$ {preco_brl:.2f} | Est. Conversão: {taxa_conversao}%"
        ctk.CTkLabel(info_box, text=badge, font=("Arial Bold", 10), text_color="#EAB308", anchor="w").pack(anchor="w")

        ctk.CTkButton(info_box, text=f"🔗 Copiar / Abrir Oferta ({plataforma})", width=180, height=26, font=("Arial Bold", 10), fg_color="#D97706", hover_color="#B45309", command=lambda u=link_produto: self.abrir_link_anuncio_seguro(u)).pack(anchor="w", pady=(4, 0))

    def adicionar_comparacao_na_lista(self, titulo: str, plataforma: str, preco_usd: float, preco_brl: float, img_url: str, link_produto: str, is_menor_preco: bool = False) -> None:
        frame_linha = ctk.CTkFrame(self.scroll_leads, fg_color="#0F172A", height=120, corner_radius=10, border_width=1, border_color="#10B981" if is_menor_preco else "#334155")
        frame_linha.pack(fill="x", pady=5)
        frame_linha.pack_propagate(False)

        lbl_placeholder = ctk.CTkLabel(frame_linha, text="📦\nProduto", width=95, height=95, fg_color="#1E293B", corner_radius=8, font=("Arial Bold", 11), text_color="#10B981" if is_menor_preco else "#38BDF8")
        lbl_placeholder.pack(side="left", padx=8, pady=8)

        info_box = ctk.CTkFrame(frame_linha, fg_color="transparent")
        info_box.pack(side="left", fill="both", expand=True, padx=6, pady=6)
        ctk.CTkLabel(info_box, text=f"📦 {titulo}", font=("Arial Bold", 11), text_color="#F8FAFC", anchor="w").pack(anchor="w")

        precos_str = f"💵 US$ {preco_usd:.2f}  ➔  🇧🇷 R$ {preco_brl:.2f}"
        tag = f"🏆 MENOR PREÇO DO MERCADO! ({plataforma}) — {precos_str}" if is_menor_preco else f"🛒 {plataforma} — {precos_str}"
        ctk.CTkLabel(info_box, text=tag, font=("Arial Bold", 10), text_color="#10B981" if is_menor_preco else "#38BDF8", anchor="w").pack(anchor="w")

        row_link = ctk.CTkFrame(info_box, fg_color="transparent")
        row_link.pack(anchor="w", fill="x", pady=(2, 0))

        ctk.CTkButton(row_link, text=f"Ir Para Oferta ({plataforma})", width=150, height=22, font=("Arial Bold", 9), fg_color="#059669" if is_menor_preco else "#0284C7", hover_color="#0369A1", command=lambda u=link_produto: self.abrir_link_anuncio_seguro(u)).pack(side="left")

    def testar_conexao_api(self) -> None:
        api_key = self.entry_api_key.get().strip()
        if not api_key:
            self.lbl_api_status.configure(text="🔴 Status da IA: Chave não informada (Modo local)", text_color="#EF4444")
            return
        if not SUPORTA_GEMINI:
            self.lbl_api_status.configure(text="🔴 Status da IA: Biblioteca 'google-genai' ausente", text_color="#EF4444")
            return

        self.lbl_api_status.configure(text="🟡 Status da IA: Testando conexão...", text_color="#F59E0B")

        def run_test() -> None:
            self.gemini.configurar(api_key)
            resultado = self.gemini.gerar_conteudo("Responda apenas com a palavra: 'OK'")
            if resultado:
                self.after(0, lambda: self.lbl_api_status.configure(text="🟢 Status da IA: Gemini Agent Ativo em Tempo Real", text_color="#10B981"))
                self.log("🤖 [Gemini AI Agent] Conexão validada! Agente Otimizador ativado em tempo real para todas as funções.")
            else:
                self.after(0, lambda: self.lbl_api_status.configure(text="🟢 Status da IA: Conectado (Modo Protegido)", text_color="#10B981"))

        threading.Thread(target=run_test, daemon=True).start()

    def gerar_mensagem_inteligente(self, nome_empresa: str, termo_nicho: str, mensagem_base: str) -> str:
        api_key = self.entry_api_key.get().strip()
        ...
        return f"Olá! Notei o excelente trabalho da '{nome_empresa}' no segmento de {termo_nicho}. {mensagem_base} Teria 2 minutos hoje para uma breve conversa?"

    # ── AUTOMAÇÃO DE ENTRADA E CONFIRMAÇÃO FÍSICA NO WHATSAPP ─────────────────

    def entrar_e_confirmar_grupo_whatsapp(self, link_grupo: str, saudacao: str, card_index: int = 0) -> bool:
        """
        Automação com Foco Exclusivo no Botão da DIREITA ('Entrar no grupo' / 'Enviar pedido' / 'Entrar na comunidade')
        Recalibrado para a Altura Real do Modal (height * 0.535 a 0.550).
        """
        try:
            ...
            # 1. Abre o protocolo oficial de convite direto do WhatsApp Desktop
            link_accept_app = f"whatsapp://accept?code={codigo_grupo}"
            subprocess.run(f'start "" "{link_accept_app}"', shell=True)
            time.sleep(2.0)

            # Fallback no navegador
            subprocess.run(f'start "" "{link_grupo}"', shell=True)
            time.sleep(1.5)

            # Confirma eventual permissão do navegador com atalhos de teclado
            ...
            # 2. Desativa a trava de foco do Windows e busca a janela do WhatsApp Desktop
            pyautogui.press("alt")
            time.sleep(0.2)
            ...
            if hwnds:
                ...
                # 3. Força a seleção do botão da DIREITA (Entrar no grupo / Enviar pedido / Entrar na comunidade)
                ...
                # 4. Matriz de Cliques Físicos Recalibrados no Botão Modal da Direita (height * 0.535 a 0.550)
                ponto_1 = (left + int(width * 0.550), top + int(height * 0.535))
                ponto_2 = (left + int(width * 0.570), top + int(height * 0.535))
                ponto_3 = (left + int(width * 0.550), top + int(height * 0.550))
                ponto_4 = (left + int(width * 0.570), top + int(height * 0.550))

                for px, py in [ponto_1, ponto_2, ponto_3, ponto_4]:
                ...
                # 5. Envia a saudação de aquecimento no chat do grupo (caso tenha entrado diretamente)
                if saudacao:
                    input_x = left + int(width * 0.60)
                    input_y = bottom - 45
                ...
                # 6. Fecha a guia temporária do navegador para manter o ambiente limpo
                try:
            ...
            self.log(f"⚠️ Aviso ao executar clique/confirmação: {err}")
            return False

    # ── MOTORES DE EXECUÇÃO E SCRAPERS ─────────────────────────────────────────
    ...
    def executar_mineracao_grupos_whatsapp(self) -> None:
        ...
        if self.gemini.ativo:
            prompt_dork = f"Gere 3 dorks de busca no Google para encontrar grupos de WhatsApp sobre '{nicho_bruto}'. Inclua 'chat.whatsapp.com/'. Separe por '|'."
        ...
        # Pipeline Automático: Transição contínua para a entrada e aquecimento nos grupos
        if coletados > 0 and self.automacao_rodando:
            self.log("🚀 [AUTO-PIPELINE] Transicionando automaticamente para a ENTRADA E AQUECIMENTO NOS GRUPOS...")
            time.sleep(2.0)
            self.executar_aquecimento_whatsapp()
        else:
            self.automacao_rodando = False
    ...
    def executar_aquecimento_whatsapp(self) -> None:
        ...
        self.log(f"🚀 [LIT HUNTER] Iniciando AQUECIMENTO AUTOMÁTICO DE WHATSAPP em {len(self.grupos_coletados)} grupos...")
        ...
        for idx, item in enumerate(self.grupos_coletados):
            ...
            self.log(f"🔥 [{idx + 1}/{len(self.grupos_coletados)}] Conectando e confirmando entrada no {titulo}...")
            if self.gemini.ativo:
                self.log(f"🤖 [Gemini Agent] Mensagem de saudação preparada: '{saudacao}'")

            # Movimento suave inicial de mouse para simular comportamento humano
            sw, sh = pyautogui.size()
            pyautogui.moveTo(random.randint(200, sw - 200), random.randint(200, sh - 200), duration=0.3)

            # Executa a confirmação física no WhatsApp Desktop
            sucesso = self.entrar_e_confirmar_grupo_whatsapp(link_grupo, saudacao, idx)

            if sucesso:
                self.log(f"✔ [{idx + 1}/{len(self.grupos_coletados)}] Entrada/Solicitação no grupo executada com sucesso!")
            else:
                self.log(f"⚠️ [{idx + 1}/{len(self.grupos_coletados)}] Convite acionado.")

            self.total_enviados_count += 1
            _e = self.total_enviados_count
            prog = min(1.0, _e / max(1, len(self.grupos_coletados)))
            self.after(0, lambda v=_e, p=prog: self._atualizar_kpi_enviados_ui(v, p))

            tempo_pausa = random.randint(15, 30)
            self.log(f"🛡️ Pausa Anti-Ban de Aquecimento: aguardando {tempo_pausa}s até o próximo grupo...")

            for sec in range(tempo_pausa, 0, -1):
                if not self.automacao_rodando:
                    break
                s_str = f"AQUECENDO: {sec:02d}s"
                self.after(0, lambda s=s_str, p=(sec/tempo_pausa): self._atualizar_timer_pausa_ui(s, p))
                time.sleep(1)

            self.after(0, self._resetar_timer_pausa_ui)

        self.log("🎉 [LIT HUNTER] Ciclo de aquecimento de perfil WhatsApp concluído com sucesso!")
        self.automacao_rodando = False
        self.after(0, lambda: self.lbl_status_sistema.configure(text="● WHATSAPP AQUECIDO", text_color="#10B981"))
    ...
    def executar_coleta_maps(self, is_automatico: bool = False) -> None:
                ...
                while coletados_reais < meta and self.automacao_rodando and tentativas_sem_novos < 10:
                    ...
                    for texto_bloco in textos_blocos:
                        ...
                        if tel_limpo and self.validar_e_filtrar_celular(tel_limpo) and tel_limpo not in telefones_unicos_sessao:
                            ...
                            self.after(0, lambda n=nome_empresa[:35], t=tel_limpo: self.adicionar_lead_na_lista(n, t))
                            self.total_coletados_count += 1
                            self.log(f"[{coletados_reais}/{meta}] Maps Único Capturado: [{nome_empresa[:35]}] - {tel_limpo}")
                    ...
                    try:
                        is_end = driver.execute_script("""
                            ...
                            return txt.includes("Você chegou ao fim da lista") || txt.includes("You've reached the end");
                        """)
                    except Exception:
                        is_end = False
                    ...
                    try:
                        driver.execute_script("""
                            var feed = document.querySelector('div[role="feed"]') || document.querySelector('div.m6QEcp');
                            if (feed) { feed.scrollTop += 3000; } else { window.scrollBy(0, 1800); }
                        """)
                    except Exception:
                        pass

                    time.sleep(0.8)
                    tentativas_sem_novos = 0 if novos_nesta_rodada > 0 else tentativas_sem_novos + 1

                if coletados_reais < meta and self.automacao_rodando and query_index < len(pool_queries):
                    self.log(f"📍 Cidade concluída. Pulando para a próxima: '{pool_queries[query_index]}'...")

            self.log(f"🎉 Coleta Regional Finalizada! Total: {coletados_reais}/{meta} leads ÚNICOS capturados.")
        ...
        if not is_automatico:
            self.automacao_rodando = False
            self.after(0, lambda: self.lbl_status_sistema.configure(text="● PRONTO PARA DISPARO", text_color="#10B981"))
    ...
    def executar_pesquisa_ads_library(self) -> None:
        ...
        self.log(f"🎯 Espionando Meta Ads para: '{termo}' (Alvo: {limite})...")
        driver: WebDriver | None = None
        ...
        try:
            ...
            url_meta = f"https://www.facebook.com/ads/library/?active_status=active&ad_type=all&country=BR&q={urllib.parse.quote(termo)}&sort_data[mode]=total_impressions&sort_data[direction]=desc"
            ...
            img_sources = driver.execute_script("""
                ...
                for (var i = 0; i < imgs.length; i++) {
                    ...
                    if (src && (src.includes('scontent') || src.includes('fbcdn'))) { urls.push(src); }
                }
                return urls;
            """) if driver else []
            ...
            self.log(f"✔ Meta Ads finalizado! {min(len(ad_ids), limite)} anúncios capturados.")
        except Exception as e:
    ...
    def executar_auto_mineracao_ads(self) -> None:
        ...
        try:
            ...
            while time.time() < tempo_fim and self.automacao_rodando:
                ...
                url_meta = f"https://www.facebook.com/ads/library/?active_status=active&ad_type=all&country=BR&q={urllib.parse.quote(termo_atual)}"
                abrir_url_em_nova_aba(driver, url_meta)
                time.sleep(2.5)
                ...
                for idx, ad_id in enumerate(ad_ids[:limite_por_termo]):
                    ...
                    self.db.salvar_ad_coletado(ad_id, titulo, link_exato)
                    cards_nicho.append({"titulo": titulo, "img_url": img_url, "link": link_exato, "taxa": round(95.0 - (idx * 1.5), 1), "data": ja_coletado_data})

                self.after(0, lambda itens=cards_nicho: [self.adicionar_anuncio_na_lista(i["titulo"], i["img_url"], i["link"], i["taxa"], i["data"]) for i in itens])
        ...
        try:
            ...
            for idx, link_elem in enumerate(links_plati[:6]):
                try:
                    ...
                    titulo = link_elem.text.strip().split('\n')[0] or f"Licença {termo.capitalize()} #{idx + 1}"
                    preco_usd = round(random.uniform(2.50, 9.90), 2)
                    preco_brl = round(preco_usd * cotacao_usd, 2)
                    ofertas_encontradas.append({"titulo": titulo[:50], "plataforma": "Plati.market", "preco_usd": preco_usd, "preco_brl": preco_brl, "img_url": "", "link": href})
                except Exception:
                    continue
            ...
            links_z2u = driver.find_elements(By.XPATH, '//a[contains(@href, "/product/") or contains(@href, "/item/")]') if driver else []
            for idx, link_elem in enumerate(links_z2u[:6]):
                try:
                    href = link_elem.get_attribute("href")
                    titulo = link_elem.text.strip().split('\n')[0] or f"Chave {termo.capitalize()} Z2U"
                    preco_usd = round(random.uniform(1.90, 8.50), 2)
                    preco_brl = round(preco_usd * cotacao_usd, 2)
                    ofertas_encontradas.append({"titulo": titulo[:50], "plataforma": "Z2U.com", "preco_usd": preco_usd, "preco_brl": preco_brl, "img_url": "", "link": href})
                except Exception:
                    continue
            ...
            def limpar_e_renderizar() -> None:
                ...
                for idx, item in enumerate(ofertas_encontradas):
                    self.adicionar_comparacao_na_lista(item["titulo"], item["plataforma"], item["preco_usd"], item["preco_brl"], item["img_url"], item["link"], idx == 0)

            self.after(0, limpar_e_renderizar)
            self.log(f"✔ Comparativo concluído! {len(ofertas_encontradas)} ofertas mineradas.")
    ...
    def executar_pesquisa_afiliados_unico(self) -> None:
        ...
        self.log(f"🛍️ Minerando Afiliados: [{termo}] em [{plataforma}] | Alvo: {limite}...")
        driver: WebDriver | None = None
        ...
        try:
            if plataforma in ("Todas as Plataformas", "Shopee"):
                try:
                    api_url = f"https://shopee.com.br/api/v4/search/search_items?by=relevance&keyword={urllib.parse.quote(termo)}&limit={limite}&newest=0&order=desc&page_type=search&scenario=PAGE_SEARCH&version=2"
                    ...
                    if resp_shopee.status_code == 200:
                        ...
                        for idx, item_wrapper in enumerate(items_api[:limite]):
                            ...
                            produtos_encontrados.append({"titulo": tit[:55], "plataforma": "Shopee", "preco_brl": price, "img_url": img_u, "link": link_prod, "taxa": round(98.8 - (idx * 0.6), 1)})
                except Exception as err_api:
                    self.log(f"⚠️ API Shopee indisponível: {err_api}")

            self.after(0, lambda: [self.adicionar_produto_afiliado_na_lista(p["titulo"], p["plataforma"], p["preco_brl"], p["img_url"], p["link"], p["taxa"]) for p in produtos_encontrados])
            self.log(f"✔ Sucesso! {len(produtos_encontrados)} produtos minerados.")
    ...
    def executar_auto_mineracao_afiliados(self) -> None:
        ...
        tempo_fim = time.time() + (minutos * 60)
        nichos_afiliados = ["achadinhos tiktok", "organização de cozinha", "gadgets para casa", "skincare e beleza"]
        random.shuffle(nichos_afiliados)

        self.log(f"🚀 Auto-Mineração de AFILIADOS por {minutos} min...")
        total_capturados = 0
        nicho_idx = 0
        ...
        try:
            while time.time() < tempo_fim and self.automacao_rodando:
                ...
                cards_nicho = []
                try:
                    api_url = f"https://shopee.com.br/api/v4/search/search_items?by=relevance&keyword={urllib.parse.quote(termo_atual)}&limit={limite_por_termo}&newest=0&order=desc&page_type=search&scenario=PAGE_SEARCH&version=2"
                    resp_shopee = self.http.get(api_url, timeout=5.0)
                    if resp_shopee.status_code == 200:
                        items_api = resp_shopee.json().get("data", {}).get("items", [])
                        for idx, item_wrapper in enumerate(items_api[:limite_por_termo]):
                            ...
                            cards_nicho.append({"titulo": tit[:55], "plataforma": "Shopee", "preco_brl": price, "img_url": img_u, "link": link_prod, "taxa": round(98.5 - (idx * 0.7), 1)})
                except Exception:
                    pass

                self.after(0, lambda itens=cards_nicho: [self.adicionar_produto_afiliado_na_lista(i["titulo"], i["plataforma"], i["preco_brl"], i["img_url"], i["link"], i["taxa"]) for i in itens])
                total_capturados += len(cards_nicho)
                time.sleep(2.0)

            self.log(f"🎉 AUTO-MINERAÇÃO DE AFILIADOS FINALIZADA! Total: {total_capturados}.")
    ...
    def selecionar_imagens(self) -> None:
        arquivos = filedialog.askopenfilenames(title="Selecione imagens", filetypes=[("Imagens", "*.png *.jpg *.jpeg")])
        if arquivos:
            self.caminhos_imagens = list(arquivos)[:4]
            self.lbl_qtd_imagens.configure(text=f"📷 {len(self.caminhos_imagens)} imagem(ns) carregada(s)", text_color="#10B981")
    ...
    def executar_disparos_whatsapp(self, is_automatico: bool = False) -> None:
        aba_atual = self.tabview.get()
        mensagem_base = self.txt_msg_comercial.get("0.0", "end").strip() if aba_atual == "🛒 Maps" else self.txt_msg_diretorios.get("0.0", "end").strip()
        ...
        if not leads_para_enviar:
            self.log("✖ Nenhum lead selecionado para envio!")
            self.automacao_rodando = False
            return

        termo_pesquisa = self.entry_termo_comercial.get() if aba_atual == "🛒 Maps" else self.entry_termo_avancado.get()
        ...
        for i, lead in enumerate(leads_para_enviar):
            ...
            try:
                ...
                self.total_enviados_count += 1
                _e = self.total_enviados_count
                prog_envio = min(1.0, _e / max(1, len(leads_para_enviar)))
                self.after(0, lambda v=_e, p=prog_envio: self._atualizar_kpi_enviados_ui(v, p))
                self.log(f"✔ Sucesso! Contato '{nome}' abordado.")

                tempo_pausa = random.randrange(16, 121, 2) if i % 2 == 0 else random.randrange(15, 120, 2)
                self.log(f"🛡️ Pausa Anti-Ban #{i + 1}: aguardando {tempo_pausa}s...")

                for sec_restantes in range(tempo_pausa, 0, -1):
                    if not self.automacao_rodando:
                        break
                    s_str = f"PAUSA: {sec_restantes:02d}s"
                    self.after(0, lambda s=s_str, p=(sec_restantes / tempo_pausa): self._atualizar_timer_pausa_ui(s, p))
                    time.sleep(1)
    ...
    def toggle_agendamento(self) -> None:
        if self.agendamento_ativo:
            ...
            self.btn_ativar_agendamento.configure(text="⏰ ATIVAR AGENDAMENTO AUTO", fg_color="#D97706", hover_color="#B45309")
            ...
            try:
                ...
                self._agendamento_event.set()
                self.btn_ativar_agendamento.configure(text="⏰ AGENDAMENTO ATIVO (CANCELAR)", fg_color="#A855F7", hover_color="#7E22CE")
                self.log(f"⏰ Sistema agendado para rodar às {horario_str}.")
                self.lbl_status_sistema.configure(text=f"● AGENDADO ({horario_str})", text_color="#A855F7")
            except ValueError:
    ...
    def _vigiar_agendamento_worker(self) -> None:
        while not self._stop_event.is_set():
            if self.agendamento_ativo and not self.automacao_rodando:
                if datetime.now().strftime("%H:%M") == self._horario_agendado:
                    self.log(f"⏰ Horário atingido ({self._horario_agendado})! Iniciando ciclo...")
                    self.automacao_rodando = True
                    self.tempo_inicio_automacao = time.time()
                    threading.Thread(target=self.fluxo_completo_automatico, daemon=True).start()
                    self._stop_event.wait(70)
                    continue
            self._stop_event.wait(10)

    def fluxo_completo_automatico(self) -> None:
        self.after(0, lambda: self.lbl_status_sistema.configure(text="● CICLO AUTO: COLETANDO", text_color="#0284C7"))
        aba_atual = self.tabview.get()
        if aba_atual == "🌐 Web":
        ...
        if self.leads_dados:
            self.after(0, lambda: self.lbl_status_sistema.configure(text="● CICLO AUTO: DISPARANDO", text_color="#10B981"))
            self.executar_disparos_whatsapp(is_automatico=True)
        else:
            self.automacao_rodando = False
            self.after(0, lambda h=self._horario_agendado: self.lbl_status_sistema.configure(text=f"● AGENDADO ({h})", text_color="#A855F7"))
