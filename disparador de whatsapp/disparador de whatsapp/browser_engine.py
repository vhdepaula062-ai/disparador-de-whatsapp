...
def check_remote_debugging_port(port: int = 9222, host: str = "127.0.0.1") -> bool:
    """Verifica se a porta de depuração do navegador já está aberta no sistema."""
    try:
...
class ProfileManager:
    ...
    def get_system_user_data_path(cls, browser: SupportedBrowsers) -> Optional[Path]:
        ...
        if platform.system() == "Windows":
            ...
            # Descoberta dinâmica baseada na árvore do executável
            try:
    ...
    def get_default_browser() -> tuple[SupportedBrowsers, str, Optional[Path]]:
        ...
        if os_name == "Windows" and SUPORTA_WINREG and winreg is not None:
            try:
                ...
                if "comet" in prog_id_lower or "comet" in cmd_lower:
                    return SupportedBrowsers.COMET, "Comet Browser", exe_path
                elif "chrome" in prog_id_lower:
                ...
                elif "opera" in prog_id_lower:
                    return SupportedBrowsers.OPERA, "Opera", exe_path
                elif "vivaldi" in prog_id_lower:
...
class BrowserCompatibility:
    """Ordem estrita de fallback prioritário para estabilidade técnica."""
    ...
    def resolve_browser(cls, preferred: Optional[SupportedBrowsers] = None) -> BrowserInfo:
        ...
        if preferred and preferred in detected and detected[preferred].is_installed:
            return detected[preferred]

        default_browser, _, _ = BrowserDetector.get_default_browser()
        if default_browser in detected and detected[default_browser].is_installed:
...
class OptionsFactory:
    """Configura opções e flags anti-automação com suporte a conexão em janela ativa CDP."""
    ...
    def build_options(cls,
                      ...
                      custom_args: Optional[List[str]] = None) -> Any:
        ...
        if b_type in [SupportedBrowsers.CHROME, SupportedBrowsers.BRAVE, SupportedBrowsers.OPERA,
            ...
            # Tenta conexão direta na janela já aberta na porta 9222 (se existir)
            if use_system_profile and check_remote_debugging_port(9222):
                logger.info("⚡ Navegador em execução detectado! Conectando à janela principal ativa...")
    ...
    def create_driver(cls, browser_info: BrowserInfo, options: Any) -> WebDriver:
        ...
        if b_type in [SupportedBrowsers.CHROME, SupportedBrowsers.BRAVE, SupportedBrowsers.OPERA,
            ...
            try:
                ...
                logger.warning(f"Selenium Manager nativo falhou para Chromium ({e1}). Tentando WebDriverManager...")
            ...
            try:
                ...
                logger.warning(f"Selenium Manager nativo falhou para Edge ({e1}). Tentando WebDriverManager...")
...
class BrowserFactory:
    ...
    def create(cls,
        ...
        options = OptionsFactory.build_options(
            ...
            custom_args=options_args
        )

        driver = DriverFactory.create_driver(browser_info, options)

        try:
            if browser_info.browser_type != SupportedBrowsers.SAFARI:
